...
int main(int argc, char **argv)
{
    ...
    if (myid == 0) {
        nx = 31;  /* Q3 requires size 31 demonstration */
    ...
    if (myid == 0) {
        printf("========================================\n");
        printf("Q3 run: parallel Poisson solver\n");
        printf("Grid size            : %d\n", nx);
    ...
    for (it = 0; it < maxit; it++) {
        exchangi1(a, nx, s, e, cart_comm, nbrleft, nbrright);
    ...
    if (myid == 0) {
        char fname[64];
        snprintf(fname, sizeof(fname), "q3_grid_%d.dat", nx);
